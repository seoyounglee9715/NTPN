...
# Number of entries in Agent State Vector = 3
# helper3.py 
# Revision Date : 23-07-07-Fri 
import math
import random
...
import numpy as np
import sys
sys.path.append('C:\\Users\\NGN\\dev\\nuscenes-devkit\\python-sdk\\nuscenes\\prediction\\models')
from backbone_n import ResNeXtBackbone
default_backbone = ResNeXtBackbone('resnext50_32x4d') # 추가
...
class MTP(nn.Module):
    ...
    def __init__(self, backbone: nn.Module = default_backbone, num_modes: int = 2,
    ...
    def forward(self, image_tensor: torch.Tensor,
        ...
        # print(mode_probabilities.shape) # [4, 2] //
        # print(mode_probabilities.ndim) # 2
        if not self.training:
            mode_probabilities = f.softmax(mode_probabilities, dim=-1)

        predictions = predictions[:, :-self.num_modes]

        return torch.cat((predictions, mode_probabilities), 1) # [batch_size, number_of_modes(2) * number_of_predictions_per_mode + number_of_modes]
    
class MTPrediction(nn.Module):
    def __call__(self, token) -> Prediction:
        ...
        return Prediction(instance, sample, np.ones(1, 6, 2), np.array([2]))
