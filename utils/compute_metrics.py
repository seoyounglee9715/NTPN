...
def compute_metrics(predictions: List[Dict[str, Any]],
    ...
    for i, prediction_str in enumerate(predictions):
        ...
        ground_truth = helper.get_future_for_agent(prediction.instance, prediction.sample,
                                                   config.seconds, in_agent_frame=False) # global, default
        #ground_truth = helper.get_future_for_agent(prediction.instance, prediction.sample,
        #                                           config.seconds, in_agent_frame=True) # local
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_root', help='Directory storing NuScenes data.', default='E:\\NuScenes\\trainval\\data\\sets\\nuscenes')
    parser.add_argument('--submission_path', help='Path storing the submission file.', default='E:\\NuScenes\\trainval\\data\\sets\\nuscenes\\output\\mb_ch3.json')
